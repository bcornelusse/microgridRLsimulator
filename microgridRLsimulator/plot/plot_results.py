...
class Plotter:
    ...
    def plot_costs(self, start, end):
        ...
        cum_fuel_cost = np.pad(np.cumsum(fuel_cost), (1,0),'constant', constant_values=0)[:-1] # Cumulative sum translated to 1 in order to align with cum total cost
        ...
        ax1.fill_between(range(len(energy_cost)),cum_fuel_cost,cum_fuel_cost+cum_curtailment_cost, label="Curtailment cost")
        ax1.fill_between(range(len(energy_cost)),cum_fuel_cost+cum_curtailment_cost,cum_fuel_cost+cum_curtailment_cost+cum_load_not_served_cost, label="Lost load cost")
        ax1.set_xticks(xticks)
        ax1.set_xticklabels(xticks_labels)
        ax1.legend(fontsize=FONT_SIZE)
        ...
        ax2.plot(energy_cost, label="Energy", drawstyle='steps-post', color = "k")
    ...
    def plot_power_mix(self, start, end):
        ...
        ax1.fill_between(range(len(generation)),generation+discharge,generation+discharge+non_steerable_productions, label="RES production", step='post')
        ax1.fill_between(range(len(generation)),generation+discharge+non_steerable_productions, generation+discharge+non_steerable_productions+imports, label="Load Shedding", step='post')
        ...
        ax2.fill_between(range(len(charge)),non_steerable_consumptions,charge + non_steerable_consumptions, label="Charges", step='post')
        ax2.fill_between(range(len(charge)),charge + non_steerable_consumptions,charge + non_steerable_consumptions+exports, label="Curtailment", step='post')
